...
AI coding agent、プロジェクトレベルの skill、rule、spec、prompt、memory、MCP ツールが日常的なエンジニアリングワークフローに入ってくるにつれて、プロジェクトには次の 2 種類の AI リソースが継続的に蓄積されます。
...
- 既存の形式を変換せずに、一般的な `skill`、`rule`、`spec`、`prompt`、`memory`、`mcp` ディレクトリを取り込む。
...
- 形式互換性優先: 既存の skill、rule、spec、prompt、memory、MCP 設定などのネイティブな整理方式との互換性を優先します。
...
- チームが共通の AI rule、spec、prompt、memory、skill を再利用したい。
